...
class C_bank extends CI_Controller
{
  ...
  */
  ...
  {    
    ...
    if($name == '' && $name == NULL){
      $success = 3;
    } else if($this->m_bank->cekBank($name) == 0){ 
      $this->m_bank->saveBank($name, $isactive, $uuid);
      if($this->m_bank->saveConfirm($uuid) == 0){ $success = 0; } else { $success = 1; }
    } else { $success = 2; }
        $data['total'] = $success;
        $data['success'] = TRUE;
        echo json_encode($data); 
  ...
  public function editBank()
  {
    ...
    if($name == '' && $name == NULL){ 
      ...
      $this->m_bank->updateBank($name, $isactive, $id);
      $success = 1;
    } else { 
    	$success = 2; 
    }
  ...
  public function searchBank()
  {
    ...
    $result = $this->m_bank->searchGridBank($username);
    foreach ($result->result() as $key => $value) {
      $data['data'][] = array(        
        'id'            => $value->id,        
        'name'          => $value->name                
