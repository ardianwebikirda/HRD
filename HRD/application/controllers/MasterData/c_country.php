...
class C_country extends CI_Controller
{
  ...
  */
  ...
  public function delCountry()
  {
    ...
    foreach($data as $row){
    	$this->m_country->deleteCountry($row->id);
    }
  ...
  {    
    ...
    if($name == '' && $name == NULL){
      $success = 3;
    } else if($this->m_country->cekCountry($name) == 0){ 
      $this->m_country->saveCountry($code, $name, $phone, $uuid);
      if($this->m_country->saveConfirm($uuid) == 0){ $success = 0; } else { $success = 1; }
    } else { $success = 2; }
        $data['total'] = $success;
        $data['success'] = TRUE;
        echo json_encode($data); 
  ...
  public function editCountry()
  {
    ...
    if($name == '' && $name == NULL){ 
      ...
      $this->m_country->updateCountry($code, $name, $phone, $id);
      $success = 1;
    } else { 
    	$success = 2; 
    }
  ...
  public function searchCountry()
  {
    ...
    $result = $this->m_country->searchGridCountry($username);
    foreach ($result->result() as $key => $value) {
      $data['data'][] = array(        
        'id'      => $value->id,
        'code'    => $value->code,        
  ...
  // { 
  //   $result = $this->m_country->printCountry();
