...
class C_religion extends CI_Controller
{
  /*
  ...
  */
  public function __construct(){
    ...
    $this->load->model('MasterData/m_religion');
  ...
  public function getReligion()
  {
    ...
    $result   = $this->m_religion->getGridReligion($start,$limit);
    $result1  = $this->m_religion->countGridReligion();
  ...
  public function delReligion()
  {
    ...
    foreach($data as $row){
    	$this->m_religion->deleteReligion($row->id);
    }
  ...
  {    
    ...
    $uuid         = $this->m_religion->getUUID();

    if($name == '' && $name == NULL){
      $success = 3;
    } else if($this->m_religion->cekReligion($name) == 0){ 
      $this->m_religion->saveReligion($name, $uuid);
      if($this->m_religion->saveConfirm($uuid) == 0){ $success = 0; } else { $success = 1; }
    } else { $success = 2; }
        $data['total'] = $success;
        $data['success'] = TRUE;
        echo json_encode($data); 
  ...
  public function editReligion()
  {
    ...
    if($name == '' && $name == NULL){ 
    	$success = 3;
    } else if($this->m_religion->cekReligionID($name, $id) == 0){ 
      $this->m_religion->updateReligion($name, $id);
      $success = 1;
    } else { 
    	$success = 2; 
    }
    ...
    */
  public function searchReligion()
  {
    $username     = ($this->input->post('name', TRUE) ? $this->input->post('name', TRUE) : '');
    $result = $this->m_religion->searchGridReligion($username);
    foreach ($result->result() as $key => $value) {
      $data['data'][] = array(        
        'id'      => $value->id,
        'name'    => $value->name            
  ...
  // { 
  //   $result = $this->m_religion->printReligion();
