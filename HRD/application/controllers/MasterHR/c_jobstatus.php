...
class C_jobstatus extends CI_Controller
{
  ...
  */
  ...
  public function delJobStatus()
  {
    ...
    foreach($data as $row){
    	$this->m_jobstatus->deleteJobStatus($row->id);
    }
  ...
  {    
    ...
    if($name == '' && $name == NULL){
      $success = 3;
    } else if($this->m_jobstatus->cekJobStatus($name) == 0){ 
      $this->m_jobstatus->saveJobStatus($name, $isactive, $uuid);
      if($this->m_jobstatus->saveConfirm($uuid) == 0){ $success = 0; } else { $success = 1; }
    } else { 
      $success = 2; 
    }
        $data['total'] = $success;
  ...
  public function editJobStatus()
  {
    ...
    if($name == '' && $name == NULL){ 
      ...
      $this->m_jobstatus->updateJobStatus($name, $isactive, $id);
      $success = 1;
    } else { 
    	$success = 2; 
    }
  ...
  {
    $name     = ($this->input->post('name', TRUE) ? $this->input->post('name', TRUE) : '');
    $result = $this->m_jobstatus->searchGridJobStatus($name);
    foreach ($result->result() as $key => $value) {
      $data['data'][] = array(        
        'id'      => $value->id,
        'name'    => $value->name            
  ...
  // { 
  //   $result = $this->m_jobstatus->printJobStatus();
