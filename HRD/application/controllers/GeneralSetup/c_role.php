...
class C_role extends CI_Controller
{
  ...
  public function getModul(){
    ...
    $result = $this->m_role->getModul($id);
    if($this->m_role->cekRoleModul($id) == 0){
      $data['data'][] = array(        
        'id'          => '',        
        'idrole'      => $id,        
        'name'        => '',                         
  ...
  public function delRole()
  {
    ...
    foreach($data as $row){
      if($this->m_role->cekRoleModul($row->id) == 0  && $this->m_role->cekRoleUser($row->id) == 0)
      {
        $this->m_role->deleteModul($row->id);
        $data['msg'] = 0;
      }
      else{
        $data['msg'] = 1;
  ...
  {    
    ...
    if($name == '' && $name == NULL){
      ...
      $this->m_role->saveRole($name, $isactive, $description, $uuid);
      if($this->m_role->saveConfirm($uuid) == 0){ $success = 0; } else { $success = 1; }
    } else { $success = 2; }
        $data['total'] = $success;
        $data['success'] = TRUE;
        echo json_encode($data); 
...
// var_dump($id, $name);
    if($name == '' && $name == NULL){ $success = 3;
      ...
      $this->m_role->updateRole($name, $isactive, $description, $id);
      $success = 1;
    } else { $success = 2; }
        $data['total'] = $success;
        $data['success'] = TRUE;
  ...
  public function saveRoleModul()
  {    
    $idrole       = ($this->input->post('idrole', TRUE) ? $this->input->post('idrole', TRUE) : '');
    $id           = ($this->input->post('id', TRUE) ? $this->input->post('id', TRUE) : '');
    $isactive1    = ($this->input->post('isactive', TRUE) ? $this->input->post('isactive', TRUE) : '');
    if($isactive1 == 'true') { $isactive = 'Y'; } else { $isactive = 'N'; }
    ...
    if($idrole == '' || $idrole == NULL){
      ...
      $success = 2; 
    } else if($this->m_role->cekIDModul($id, $idrole) == 0){ 
      $this->m_role->saveRoleModul($id, $idrole, $isactive, $iscreate, $isupdate, $isdelete, $isprocess, $uuid);
      if($this->m_role->saveConfirmModul($uuid) == 0){ $success = 4; 
      } else { $success = 0; }
    } else { $success = 3; }

      $result = $this->m_role->getModul($idrole);
      foreach ($result->result() as $key => $value) {
          $data['data'][] = array(        
            'id'          => $value->id,        
            'idrole'      => $idrole,        
  ...
  public function delRoleModul()
  {
    $id   = ($this->input->post('id', TRUE) ? $this->input->post('id', TRUE) : '');
    $idrole   = ($this->input->post('idrole', TRUE) ? $this->input->post('idrole', TRUE) : '');

      $this->m_role->deleteRoleModul($id);
      $result = $this->m_role->getModul($idrole);
